...
from base.models import Upload_Assignment, Assignment

def upload_assignment_list(request,id):
    users = Upload_Assignment.objects.filter(Assignment_id=id).values('update_by').distinct()
    return render(request, 'assignment/upload_assignment_list.html', {'assignments': users})
...
def upload_assignment_create(request,qst_id):
    ...
    return render(request, 'assignment/upload_assignment_create.html',{'data':obj,"file":file})
